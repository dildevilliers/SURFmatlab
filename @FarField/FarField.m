...
%             [obj.th, obj.ph] = obj.getphth();
        ...
        function [u, v, w] = getUV(obj)
            switch obj.gridTypeBase
                case 'PhTh'
                    [u,v,w] = FarField.PhTh2UV(obj.xBase,obj.yBase);
                case 'UV'
                    u = obj.xBase;
                    v = obj.yBase;
                    w = sqrt(1 - u.^2 - v.^2);
                case 'AzEl'
                    [u,v,w] = FarField.AzEl2UV(obj.xBase,obj.yBase);
                case 'ElAz'
                    [u,v,w] = FarField.ElAz2UV(obj.xBase,obj.yBase);
                case 'TrueView'
                    [u,v,w] = FarField.TrueView2UV(obj.xBase,obj.yBase);
                case 'ArcSin'
                    [u,v,w] = FarField.ArcSin2UV(obj.xBase,obj.yBase);
            end
        end
        ...
        function [Xg, Yg] = getTrueView(obj)
            switch obj.gridType
                ...
                otherwise
                    [u,v,w] = getUV(obj);
                    [Xg,Yg] = FarField.UV2TrueView(u,v,w);
            end
        end
        ...
        function [asinu, asinv] = getArcSin(obj)
            switch obj.gridType
                ...
                otherwise
                    [u,v,~] = getUV(obj);
                    [asinu,asinv] = FarField.UV2ArcSin(u,v,w);
            end
        end
...
%                     Eaz(iPole,:) = Eth(iPole,:) + Eph(iPole,:);
    ...
    methods (Static = true)
       %% Farfield reading methods
        ...
        function [u,v,w] = ElAz2UV(ep,al)
            u = real(sin(al));
            v = real(cos(al).*sin(ep));
            w = real(cos(al).*cos(ep));
        end
        
        function [u,v,w] = TrueView2UV(x,y)
            Th = sqrt(x.^2 + y.^2);
            Ph = atan2(y,x);
            u = real(sin(Th).*cos(Ph));
            v = real(sin(Th).*sin(Ph));
        ...
        function [u,v,w] = ArcSin2UV(x,y)
            u = sin(x);
            v = sin(y);
            w = real(sqrt(sin(x).^2 + sin(y).^2));
        end
        
        function [ph,th] = UV2PhTh(u,v,w)
            ph = real(atan2(v,u));
            th = real(acos(w));
        end
        
        function [az,el] = UV2AzEl(u,v,w)
            el = real(asin(v));
            az = real(atan2(u,w));
        end
        
        function [ep,al] = UV2ElAz(u,v,w)
            al = real(asin(u));
            ep = real(atan2(v,w));
        end
        
        function [Xg,Yg] = UV2TrueView(u,v,w)
            Ph = atan2(v,u);
            Th = acos(w);
            Xg = real(Th.*cos(Ph));
            Yg = real(Th.*sin(Ph));
        ...
        function [asinu,asinv] = UV2ArcSin(u,v,w)
            asinu = real(asin(u));
            asinv = real(asin(v));
        end
    end
